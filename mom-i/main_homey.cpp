...
{ 
	...
	*/
	...
	{
		HOMEY_MEDIA_INFO_S homey_media;
		char rtsp_name[128] = {0};
		int audio_codec = 0, audio_channel = 0, audio_freq = 0;
		int rtsp_port = 554;
		std::string rtsp_user, rtsp_pwd;

		memset(&homey_media, 0, sizeof(homey_media));
		SK_INI_GetRtspInfo(rtsp_port, rtsp_user, rtsp_pwd);
		if(rtsp_port <= 0 || rtsp_port > 65535) rtsp_port = 554;

		if(get_rtsp_name(MASTER_INDEX, rtsp_name) && rtsp_name[0])
		{
			strncpy(homey_media.rtsp_path, rtsp_name, sizeof(homey_media.rtsp_path) - 1);
		}
		else
		{
			strncpy(homey_media.rtsp_path, "live0", sizeof(homey_media.rtsp_path) - 1);
		}

		get_audio_param(audio_codec, audio_channel, audio_freq);
		homey_media.rtsp_port = rtsp_port;
		homey_media.width = input_infor.m_width;
		homey_media.height = input_infor.m_height;
		homey_media.fps = input_infor.m_fps;
		homey_media.audio_freq = audio_freq;
		homey_media.audio_channels = audio_channel;
		homey_media.audio_bits = 16;
		SK_HOMEY_updateMediaInfo(&homey_media);
		SK_HOMEY_startService();
	}
	...
	{	
		...
		{			
			...
			HOMEY_SENSOR_VALUE_S homey_sensor;
			homey_sensor.temperature = sensor_value.temp;
			homey_sensor.humidity = sensor_value.humidity;
			homey_sensor.volume = sensor_value.volume;
			homey_sensor.light = sensor_value.light;
			homey_sensor.thermal_temperature = sensor_value.ircam_temp;
			SK_HOMEY_updateSensorValue(&homey_sensor);
		}
