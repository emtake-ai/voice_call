...
Both inputs use the same SeniorService mode logic:
   CHAT_MODE:
       normal chat
       start query / check my condition -> QUESTION_MODE
       refresh / reload -> refresh sensor data
   QUESTION_MODE:
       user answer -> next question
       after max questions -> summary -> CHAT_MODE

Then answer/system text is spoken:
   text -> Piper TTS -> PCM -> Opus/RTP/UDP -> IoT speaker
...
try:
    from senior_service import SeniorService, ServiceMode
...
class SeniorVoiceCallService:
    ...
    def start(self):
        ...
        print("  start query / check my condition -> QUESTION_MODE", flush=True)
        print("  stop / cancel / back -> cancel QUESTION_MODE", flush=True)
        print("  refresh / reload -> reload sensor data", flush=True)
        print("  exit / quit -> stop service\n", flush=True)
        ...
        if self.senior.abnormal_status.get("has_abnormal"):
            msg = "Abnormal sensor status detected. Entering QUESTION_MODE first."
            print(msg, flush=True)
            first_question = self.start_question_mode()
            self.answer_queue.put(msg + " " + first_question)
    ...
    def text_reasoning_loop(self):
        while not self.stop:
            ...
            response = self.handle_text_with_senior_modes(user_text)
            response = response.strip()

            if response:
                print(f"Assistant/System: {response}\n", flush=True)
                self.answer_queue.put(response)

    def handle_text_with_senior_modes(self, user_text: str) -> str:
        text_lower = user_text.strip().lower()

        # QUESTION_MODE: user_text is treated as an answer to the current question.
        if self.senior.mode == ServiceMode.QUESTION_MODE:
            if text_lower in ("stop", "cancel", "back"):
                self.senior.mode = ServiceMode.CHAT_MODE
                self.senior.question_manager.reset()
                return "QUESTION_MODE cancelled. Returning to CHAT_MODE."

            self.senior.question_manager.save_answer(user_text)

            if self.senior.question_manager.is_finished():

                summary_text = self.finish_question_mode_text()

                self.save_question_summary(summary_text)
            ...
            question = self.senior.question_manager.next_question(self.senior.abnormal_status)
            return f"QUESTION_MODE {self.senior.question_manager.step}/{self.senior.question_manager.max_questions}. {question}"

        # CHAT_MODE: detect intent first. This is the important fix.
        intent = self.senior.intent_manager.detect(user_text)

        if intent == Intent.START_QUERY:
            prefix = "Starting QUESTION_MODE based on current sensor data."
            if not self.senior.abnormal_status.get("has_abnormal"):
                prefix += " No abnormal sensor status detected, but I will still ask general check questions."
            question = self.start_question_mode()
            return f"{prefix} QUESTION_MODE {self.senior.question_manager.step}/{self.senior.question_manager.max_questions}. {question}"

        if intent == Intent.REFRESH:
            self.senior.fetch_and_check()
            return self.abnormal_status_text()

        if intent == Intent.EXIT:
            self.stop = True
            return "Service stopped."

        return self.senior.chat_response(user_text)

    def start_question_mode(self) -> str:
        self.senior.mode = ServiceMode.QUESTION_MODE
        self.senior.question_manager.reset()
        return self.senior.question_manager.next_question(self.senior.abnormal_status)

    def finish_question_mode_text(self) -> str:
        lines = []
        lines.append("The condition check is complete.")

        issues = self.senior.abnormal_status.get("issues", [])
        if issues:
            lines.append("Abnormal sensor issues:")
            for issue in issues:
                lines.append(
                    f"- {issue.get('device', 'unknown device')} / "
                    f"{issue.get('category', 'unknown category')} / "
                    f"{issue.get('priority', 'unknown priority')}: "
                    f"{issue.get('message', '')}"
                )
        else:
            lines.append("No abnormal sensor issues were detected.")

        if self.senior.question_manager.answers:
            lines.append("Caregiver answers:")
            for item in self.senior.question_manager.answers:
                lines.append(f"- Q: {item.get('question', '')}")
                lines.append(f"  A: {item.get('answer', '')}")

        lines.append("Returning to CHAT_MODE.")
    ...
    def save_question_summary(self, summary_text):
        os.makedirs("logs", exist_ok=True)

        with open(
            "logs/condition_log.txt",
            "a",
            encoding="utf-8"
        ) as f:
            f.write("\n")
    ...
    def abnormal_status_text(self) -> str:
        if not self.senior.abnormal_status.get("has_abnormal"):
            return "Relay Server data refreshed. No abnormal sensor status detected."

        lines = ["Relay Server data refreshed. Abnormal sensor status detected."]
        for issue in self.senior.abnormal_status.get("issues", []):
            lines.append(
                f"- {issue.get('device', 'unknown device')} / "
                f"{issue.get('category', 'unknown category')} / "
                f"{issue.get('priority', 'unknown priority')}: "
...
def parse_args():
    parser = argparse.ArgumentParser(
        description="Integrated IoT voice assistant: IoT MIC/console text -> SeniorService modes -> Piper -> RTP/Opus"
    )
